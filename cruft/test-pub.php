...
$hosts = isset($argv[2]) ? explode(',',$argv[2]) : array('127.0.0.1');
...
for ($i=1; $i<=$n; $i++) {
     $nsq->publish('test', new nsqphp\Message\Message(json_encode(array('msg' => $i, 'run' => "struggle"))));
    echo "Published $i to `test`\n";
}
